...
*dk morse
      ...
      include 'size.h'
      include 'pcom.h'
      real ra(3,8,*), rshl(*), c1(nr+1), c2(nr+1), c3(nr+1)
      common /heat/ htop, hbot, hrad, heat, hshr, hvol, hnet, tnrm,
     &              tav(nr+1), qc(nr)
      common /eos1/ rhorf(nr+1), tmprf(nr+1), drhdp(nr+1), grv(nr+1)
      common /eos2/ rhocv(nr+1), gamma(nr+1), alpha(nr+1), prf(nr+1),
...
!      if(index.eq.13) call subduct(temp,f,map,map2)
      ...
      if(index.eq.15) call tomog(temp,f)
		...
		if(index==17) call tempcube(temp) 

      if(index.ge.20 .and. index.lt.50) call randtemp(temp,tav,index)
...
*dk bdcvinit
	...
	subroutine tempcube(temp)
!	This routine changes the input scalar field for a
!	certain number of gridpoints by adding a specified
!	value. Input are kstart/kend, diamond and radial
!	layer and the perturbation value.
!	Peter: JULY10 2001 same as vecchange3, but for mt=nr
	implicit none
	
	include 'size.h'
	include 'pcom.h'

	integer k, ir, i1, i2, i3
	real temp(nt+1,nt+1,nd,nr+1)
	real value

	value=200.0
	if(mynum==17.or.mynum==31) then
	do ir = 20,36
		do i3 = 1,nd
			do i2 = 1,nt+1
				do i1 = 1,nt+1
					temp(i1,i2,i3,ir)=temp(i1,i2,i3,ir)+value
				enddo
			enddo
		enddo
	enddo
